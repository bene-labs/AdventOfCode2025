...
def count_paths_to(nodes: dict[str, list[str]], start: str, end: str) -> int:
    
    @cache
    def _count_paths_to_rec(current: str, goal: str)-> int:
        if current == goal:
            return 1
        return sum([_count_paths_to_rec(next_node, goal) for next_node in nodes[current]])

    return _count_paths_to_rec(start, end)


def count_paths_along(nodes: dict[str, list[str]], start: str, end: str, to_visit: frozenset[str]) -> int:
    ...
    def _count_paths_along_rec(current: str, goal: str, must_visit: frozenset[str])-> int:
        ...
        return sum([_count_paths_along_rec(next_node, goal, must_visit) for next_node in nodes[current]])

    return _count_paths_along_rec(start, end, to_visit)


def get_nodes(input_path: str)->dict[str, list[str]]:
    ...
    with open(input_path, 'r', encoding='utf-8-sig') as input_file:
        for line in input_file.read().splitlines():
            ...
            if name in unique_nodes:
                unique_nodes[name] += connected_nodes
            else:
                unique_nodes[name] = connected_nodes
